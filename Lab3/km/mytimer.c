...
// device state (single timer variant per spec)
struct mytimer_dev {
    ...
    unsigned long loaded_j;              // module load time (jiffies)
...
static const struct proc_ops mytimer_proc_ops = {
    .proc_open    = mytimer_proc_open,
    .proc_read    = seq_read,
